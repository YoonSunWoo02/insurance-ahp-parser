...
매우 중요 — 각주(주1), 주2) 등) 참조로만 금액이 나오는 경우, 각주 정의를 찾아서 채워라:
"...외래의 보험가입금액주2)의 한도 내에서 보상"처럼 본문에는 "주1)", "주2)" 같은
각주 번호만 있고 실제 숫자가 안 보일 수 있다. 이 경우 절대 amount를 null로
넘기지 말고, 같은 조항 안 다른 곳에 있는 "주2) 회당 20만원으로 합니다." 같은
각주 정의 문장을 찾아 그 금액을 amount로 채워라. 각주 정의가 조항 안 어디에도
없을 때만 null로 둔다.
