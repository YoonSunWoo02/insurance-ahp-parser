...
**d) ✅ 해결됨 — enum 검증의 `coverage_type` 조합값 완화 (2026-07-11)**
- 초기 구현은 허용값 **정확 일치**만 인정해, GPT가 정당하게 반환하는 `coverage_type="입원 | 통원"`(입원·통원 모두 보장) 조합값도 "불명"으로 치환·감점했다(삼성화재 실손 다수).
- **완화**: `coverage_type`(및 `PIPE_COMBO_FIELDS`)에 한해 `"|"`로 분해한 각 조각이 모두 허용값(불명 제외)이면 유효로 인정한다(`_is_valid_enum`).
- **버그는 계속 차단**: 선택지 문자열 통째 반환(`"입원 | 통원 | 수술 | 불명"`)은 조합에 **"불명"이 섞여 있으므로** 무효로 판정된다(불명은 단독 전용). `contract_type`/`benefit_type`은 조합이 정당하지 않아 그대로 엄격 적용.
